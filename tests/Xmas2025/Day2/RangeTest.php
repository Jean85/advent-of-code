...
class RangeTest extends TestCase
{
    ...
    #[DataProvider('rangeWithBetterCheckDataProvider')]
    public function testGetWrongIdWithBetterCheck(int $start, int $end, array $expectedWrongIds): void
    {
        foreach ($expectedWrongIds as $expectedWrongId) {
            $this->assertGreaterThanOrEqual($start, $expectedWrongId);
            $this->assertLessThanOrEqual($end, $expectedWrongId);
        ...
        $this->assertEquals($expectedWrongIds, iterator_to_array($range->getWrongIdsWithBetterCheck()));
    }

    /**
     * @return array{int, int, int[]}[]
     */
    public static function rangeWithBetterCheckDataProvider(): array
    {
        return [
            [11, 22, [11, 22]],
            [95, 115, [99, 111]],
            ...
            // edge case - no numbers in between the repeated pattern
            [21_232, 21_232, []],
        ];
    }
}
